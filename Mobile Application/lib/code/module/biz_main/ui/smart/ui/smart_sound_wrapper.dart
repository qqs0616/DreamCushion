...
class SmartSoundWrapper {
  ...
  List<FlSpot> soundListFlSpot = List.empty(growable: true);

  SmartSoundWrapper(this.themeData, this.soundList, this.xSize) {
    bottomName.clear();
    soundListFlSpot.clear();
    int index = 0;
    for (var element in soundList) {
      soundListFlSpot.add(FlSpot(index.toDouble(), element.y));
  ...
  List<LineChartBarData> getLineBarsData() {
    return [
      getLineChartBarData(soundListFlSpot),
    ];
  }
